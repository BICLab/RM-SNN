...
from DVS128_Gesture.data_process.events_timeslices import *
from DVS128_Gesture.data_process.transforms import *
...
class DVSGestureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            with h5py.File(
                root_test + os.sep + self.files_train[idx],
                "r",
                swmr=True,
                libver="latest",
            ) as f:
            ...
            data = my_chunk_evs_pol_dvs(
                data=data, dt=self.dt, T=self.chunk_size, size=self.size, ds=self.ds
            )
            if self.is_spike:
            ...
            with h5py.File(
                root_test + os.sep + self.files_test[idx],
                "r",
                swmr=True,
                libver="latest",
            ) as f:
                data, target = sample_test(
                    f, T=self.chunk_size, clip=self.clip, dt=self.dt
                )
...
def sample_train(hdf5_file, T=60, dt=1000, is_train_Enhanced=False):
    ...
    tmad = get_tmad_slice(
        hdf5_file["times"][()], hdf5_file["addrs"][()], start_time, T * dt
    )
    tmad[:, 0] -= tmad[0, 0]
    return tmad[:, [0, 3, 1, 2]], label


def sample_test(hdf5_file, T=60, clip=10, dt=1000):
    label = hdf5_file["labels"][()]
    ...
    tmad = get_tmad_slice(
        hdf5_file["times"][()], hdf5_file["addrs"][()], tbegin, tend - tbegin
    )
    tmad[:, 0] -= tmad[0, 0]
    ...
    if clip * T * dt - (end_time - start_time) > 0:
        overlap = int(np.floor((clip * T * dt - (end_time - start_time)) / clip))
        for j in range(clip):
        ...
        overlap = int(np.floor(((end_time - start_time) - clip * T * dt) / clip))
        for j in range(clip):
            start_point.append(j * (T * dt + overlap))
            if start_point[-1] + T * dt > end_time:
                diff = start_point[-1] + T * dt - end_time
